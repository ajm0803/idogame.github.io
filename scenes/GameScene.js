...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.touchIndicator = this.add.circle(0, 0, 30, 0xffffff, 0.3).setVisible(false).setDepth(100);
    ...
    createHUD() {
        const hudStyle = { fontFamily: '"Press Start 2P"', fontSize: '20px', color: '#ffffff', stroke: '#000000', strokeThickness: 4 };
        ...
        this.levelText = this.add.text(this.scale.width - 40, 0, 'THE PARK', { ...hudStyle, fontSize: '18px', color: '#ff69b4' }).setOrigin(1, 0);
    ...
    createMobileControls() {
        ...
        const { width, height } = this.scale;
        const btnSize = 110; const marginX = 40; const marginY = 40; const spacing = 20;
        const btnAlpha = 0.4; const btnAlphaPressed = 0.8;
        
        const createBtn = (x, y, label, color, onPress, onRelease) => {
            const container = this.add.container(x, y).setScrollFactor(0).setDepth(2000);
            const circle = this.add.circle(0, 0, btnSize / 2, color, btnAlpha).setInteractive({ useHandCursor: true });
            const text = this.add.text(0, 0, label, { fontSize: '40px', color: '#fff', stroke: '#000', strokeThickness: 4 }).setOrigin(0.5);
            container.add([circle, text]);
            circle.on('pointerdown', () => { circle.setAlpha(btnAlphaPressed); circle.setScale(0.9); onPress(); });
            const release = () => { circle.setAlpha(btnAlpha); circle.setScale(1); onRelease(); };
    ...
    collectItem(player, item) {
        ...
        if (type === 'rose' || type === 'chocolate' || type === 'ring' || type === 'steak' || type === 'brandy') {
            ...
            const scorePopup = this.add.text(player.x, player.y - 60, labelMap[type], { fontFamily: '"Press Start 2P"', fontSize: '18px', color: colorMap[type], stroke: '#000000', strokeThickness: 3 }).setOrigin(0.5).setDepth(500);
            ...
            if (collectedAll && !this.hasKey) {
                ...
                const arrow = this.add.text(this.player.x + 120, this.player.y - 80, '➡️⛪', { fontSize: '40px' }).setDepth(500);
    ...
    hitByDog(player, dog) {
        ...
        if (this.itemsCollected[item2Name] > 0) {
            ...
            const popup = this.add.text(player.x, player.y - 80, `${item2Emoji} Here boy!`, { fontFamily: '"Press Start 2P"', fontSize: '16px', color: item2Name === 'steak' ? '#ff4444' : '#8B4513', stroke: '#000000', strokeThickness: 3 }).setOrigin(0.5).setDepth(500);
            ...
            const eatText = this.add.text(dog.x, dog.y - 60, '🐕 Yum!', { fontFamily: '"Press Start 2P"', fontSize: '14px', color: '#ffd700', stroke: '#000000', strokeThickness: 3 }).setOrigin(0.5).setDepth(500);
            ...
            const popup = this.add.text(player.x, player.y - 80, `🐕 No ${item2Label}!`, { fontFamily: '"Press Start 2P"', fontSize: '14px', color: '#ff4444', stroke: '#000000', strokeThickness: 3 }).setOrigin(0.5).setDepth(500);
            this.tweens.add({ targets: popup, y: popup.y - 80, alpha: 0, duration: 1200, onComplete: () => popup.destroy() });
    ...
    enterChurch(player, church) {
        ...
        if (this.hasKey && !this.churchDoorOpen) {
            ...
            this.tweens.add({ targets: doorGlow, displayHeight: 130, displayWidth: 80, alpha: 1, duration: 800, ease: 'Power2', onComplete: () => {
                ...
                for (let i = 0; i < 10; i++) {
                    const s = this.add.text(this.church.x + Phaser.Math.Between(-30, 30), this.church.y - Phaser.Math.Between(40, 140), Phaser.Math.RND.pick(['✨', '💒', '⭐', '🌟', '💫']), { fontSize: Phaser.Math.Between(16, 28) + 'px' }).setDepth(500);
            ...
            for (let i = 0; i < 10; i++) {
                const sparkle = this.add.text(keyX + Phaser.Math.Between(-80, 80), keyY + Phaser.Math.Between(-60, 60), Phaser.Math.RND.pick(['✨', '🔑', '⭐', '💫']), { fontSize: Phaser.Math.Between(20, 36) + 'px' }).setDepth(500).setAlpha(0);
    ...
    collectPowerUp(player, powerUp) {
        ...
        if (powerType === 'speed') {
            ...
            this.speedTrailTimer = this.time.addEvent({ delay: 150, callback: () => {
                ...
                const sparkle = this.add.text(this.player.x, this.player.y + 20, '✨', { fontSize: '18px' }).setDepth(150).setAlpha(0.8);
    ...
    update(time, delta) {
        ...
        // Direct touch movement logic (move towards finger)
        let directTouchLeft = false;
        let directTouchRight = false;
        let directTouchRun = false;
        
        const pointer = this.input.activePointer;
        const isUsingButtons = this.touchLeft || this.touchRight || this.touchJump || this.touchRun;
        
        if (pointer.isDown && !isUsingButtons) {
            const worldX = pointer.worldX;
            const dx = worldX - this.player.x;
            
            // Show indicator where they touch for movement feedback
            this.touchIndicator.setPosition(worldX, pointer.worldY).setVisible(true);
            const pulse = 1 + Math.sin(time * 0.01) * 0.2;
            this.touchIndicator.setScale(pulse);

            if (Math.abs(dx) > 40) { // Deadzone
                if (dx < 0) directTouchLeft = true;
                else directTouchRight = true;
                // Run if touch is far away (more than 180 pixels)
                if (Math.abs(dx) > 180) directTouchRun = true;
            }
        } else {
            this.touchIndicator.setVisible(false);
        }

        const wasInAir = !this.player.body.wasTouching.down; const isJumping = !this.player.body.touching.down; const onGround = this.player.body.touching.down;
        const isRunning = (this.cursors.shift && this.cursors.shift.isDown) || this.touchRun || directTouchRun; 
        const speedMultiplier = this.isSpeedBoosted ? 1.5 : 1; const speed = (isRunning ? 700 : 450) * speedMultiplier;
        let nextAnim = 'idle'; 
        const leftInput = this.cursors.left.isDown || this.touchLeft || directTouchLeft; 
        const rightInput = this.cursors.right.isDown || this.touchRight || directTouchRight; 
        ...
        if (jumpInput && onGround) { this.player.setVelocityY(-1350); nextAnim = 'jump'; audioManager.playJump(); this.touchJump = false; this.emitDustBurst(6, 160); }
        if (isJumping) { nextAnim = 'jump'; this.player.setAngle(this.player.body.velocity.y * 0.03); } else this.player.setAngle(0);
        if (wasInAir && onGround) this.emitDustBurst(8, 200);
